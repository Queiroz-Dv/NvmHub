...
public interface INvmService
{
    ...
    /// <summary>Retorna a versão do Node.js atualmente ativa ("nvm current").</summary>
    Task<string?> GetCurrentVersionAsync(CancellationToken ct = default);

    Task<string?> GetNvmVersionAsync(CancellationToken ct = default);
    Task<OperationResult<IEnumerable<string>>> ListRemoteVersionsAsync(CancellationToken ct = default);
}
